...
export const generateQuizResultPDF = (result: QuizResult, userName: string) => {
  ...
  pdf.text('UniHub - Quiz Results Report', 105, 20, { align: 'center' });
  ...
  pdf.text(`Date: ${date.toLocaleDateString('en-US', {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
    hour: '2-digit',
  ...
  // Quiz type
  pdf.text(`Quiz Type: ${result.quiz_type === 'initial' ? 'Initial Quiz' : 'Extended Quiz'}`, 20, 49);
  ...
  pdf.text('Main Compatibility', 20, 63);
  ...
  pdf.text(`Compatibility Score: ${Math.round(result.compatibility_score)}%`, 20, 80);
  ...
  if (result.matched_universities && result.matched_universities.length > 0) {
    ...
    pdf.setFontSize(14);
    pdf.setTextColor(33, 150, 243);
    pdf.text('Recommended Universities:', 20, currentY);
  ...
  pdf.text('UniHub - AI-powered Academic Guidance Platform', 105, 285, { align: 'center' });
  pdf.text('www.unihub.ro', 105, 290, { align: 'center' });
  
  // Save the PDF
  const fileName = `UniHub_Results_${result.main_match_field.replace(/\s+/g, '_')}_${date.toISOString().split('T')[0]}.pdf`;
  pdf.save(fileName);
};
